/* Visual acceptance for the QB Spatial Hero composition (lab). Not part of `npm test`: it needs Playwright's Chromium.
   ...
   positions on three iPhone viewports (the in-app browser is the short one), and asserts the sculpture holds:
     the wordmark never reaches the rule, QUIET stays interlocked with BANDS, BANDS clears the rule but stays with it,
...
const LIMITS = { gapQL: 16, ovQB: 0, feetMin: 8, feetMax: 60, gapLW: 6 };   // ovQB 0: the two words may touch, never separate; feet: BANDS clears the rule but stays with it   // px, CSS
...
for (const [w, h, name] of VIEWPORTS) {
  ...
  const worst = { gapQL: Infinity, ovQB: Infinity, feetMin: Infinity, feetMax: -Infinity, gapLW: Infinity };
  for (const [vx, vy, p] of grid) {
    ...
    const m = await pg.evaluate(() => {
      ...
      return { gapQL: ln.top - q.bottom, ovQB: q.bottom - bd.top, feet: ln.top - bd.bottom, gapLW: wh.top - ln.bottom };
    });
    worst.gapQL = Math.min(worst.gapQL, m.gapQL); worst.ovQB = Math.min(worst.ovQB, m.ovQB); worst.feetMin = Math.min(worst.feetMin, m.feet); worst.feetMax = Math.max(worst.feetMax, m.feet); worst.gapLW = Math.min(worst.gapLW, m.gapLW);
    const bad = [];
    ...
    if (m.feet < LIMITS.feetMin || m.feet > LIMITS.feetMax) bad.push(`BANDS feet ${m.feet.toFixed(0)}px above the rule`);
  ...
  console.log(`${name} ${w}x${h}: worst gapQL ${worst.gapQL.toFixed(0)} (>=${LIMITS.gapQL})  ovQB ${worst.ovQB.toFixed(0)} (>=${LIMITS.ovQB})  feet ${worst.feetMin.toFixed(0)}..${worst.feetMax.toFixed(0)} (${LIMITS.feetMin}..${LIMITS.feetMax})  gapLW ${worst.gapLW.toFixed(0)} (>=${LIMITS.gapLW})`);
  await ctx.close();
}
